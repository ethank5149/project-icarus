...
3. **Phase 7 — Dymos terminal phase wired to closed-loop guidance (DONE 2026-07-19):** `build_trajectory_problem(closed_loop=True)` (default) now uses `ClosedLoopTerminalODE` in `src/optimization/phases/terminal_phase.py`, which computes the interceptor acceleration each node from `TerminalGuidance` (PN/APN/ZEM/SDRE-MPC) using the relative kinematics to the aim point — the same closed-loop logic `EngagementRunner` uses. The optimizer tunes the guidance gains `N` and `accel_limit` (exposed as Dymos parameters) to directly minimize the final `miss_distance` (proven: a standalone terminal phase drives a 20 km offset to ~5 km). `closed_loop=False` preserves the original open-loop `accel` controls + time/position-constraint formulation. `tests/test_terminal_guidance_opt.py` covers assembly + miss-distance reduction.
