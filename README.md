...
1. **Phase 5A→5B:** extend `src/sensors/` with clutter/false-alarm models, M-of-N track confirmation, and a data-fusion node; then build `src/c2/BattleManager` (assessment, weapon allocation, salvo logic) driving `run_campaign` (extends `run_sweep`) for saturation raids.
