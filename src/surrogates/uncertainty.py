...
class UncertaintyPropagator:
    ...
    def propagate(self, nominal_state, sigma_cd, sigma_cl, sigma_cm, eom_func):
        """
        ...
        sigma_cd, sigma_cl, sigma_cm : float
            1-sigma coefficient uncertainties.
        eom_func : callable
            Function(cd, cl, cm) -> miss_distance.
        ...
        samples = self.rng.normal(
            loc=[0.0, 0.0, 0.0],
            scale=[sigma_cd, sigma_cl, sigma_cm],
    ...
    def sobol_indices(self, nominal_state, sigma_cd, sigma_cl, sigma_cm, eom_func, n_samples=1000):
        """
        Cheap sensitivity indices via elementary effects.
        Returns relative sensitivity of miss distance to each coefficient.
        """
        base_miss, _ = self.propagate(nominal_state, sigma_cd, sigma_cl, sigma_cm, eom_func)
        factors = [1.0, 1.05, 0.95]
        sensitivities = []
        for sigma, factor in zip([sigma_cd, sigma_cl, sigma_cm], factors):
            scaled = sigma * factor
            m, _ = self.propagate(nominal_state, scaled if factor > 1 else sigma_cd, sigma_cl if factor > 1 else scaled, sigma_cm, eom_func)
