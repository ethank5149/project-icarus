...
class SarmatTerminalODE(om.ExplicitComponent):
    """Reentry / terminal phase ODE for RS-28 Sarmat.

    Pure ballistic descent under gravity + drag.  The miss distance to the
    defended target is computed each node so the optimizer can minimize it
    directly.
    """
    ...
    def setup(self):
        ...
        self.add_input("m", val=np.full(nn, 800.0))
        ...
        self.add_output("miss_distance", val=np.full(nn, 1e6))
    ...
    def compute(self, inputs, outputs):
        ...
        for i in range(nn):
            ...
            outputs["dm_dt"][i] = 0.0
            outputs["miss_distance"][i] = float(np.linalg.norm(r[i] - target_r[i]))
